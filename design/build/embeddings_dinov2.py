...
'''
embeddings.py에서 이미 저장된 이미지를 불러와 DINOv2-L으로 임베딩한다.

- 원본 이미지  → "data/images"         → "data/embeddings_original_dinov2"
- 스케치 이미지 → "data/images_sketch"  → "data/embeddings_sketch_dinov2"
'''

# DINOv2-L 모델 로드
print("DINOv2-L 모델 로딩 중...")
...
# 스케치 임베딩 저장할 폴더
EMBEDDING_OUTPUT = f"{BASE_DIR}/embeddings_sketch_dinov2/2024-2026"
# 원본 임베딩 저장할 폴더
EMBEDDING_OUTPUT_ORIGINAL = f"{BASE_DIR}/embeddings_original_dinov2/2024-2026"
# 에러 기록 파일
...
for idx, filename in enumerate(list, 1):
    ...
    image_name = data.get('image', {}).get('imageName', 'image.jpg') #사진 이름
    image_path = data.get('image', {}).get('imagePath') #사진 경로
    design_id = data.get('design_id', 'unknown') #디자인 ID

    image_number_pre = data.get('image', {}).get('number', '1') #도면 번호
    
    #스케치 임베딩 및 원본 임베딩이 존재하는지 확인 
    sketch_done   = os.path.exists(os.path.join(EMBEDDING_OUTPUT,          f"{design_id}-{image_number_pre}_embedding.json"))
    original_done = os.path.exists(os.path.join(EMBEDDING_OUTPUT_ORIGINAL, f"{design_id}-{image_number_pre}_embedding.json"))

    # 둘 다 존재하면 skip 
    if sketch_done and original_done:
        print(f"SKIP: 이미 처리됨 → {design_id}-{image_number_pre}")
        continue
